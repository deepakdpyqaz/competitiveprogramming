...
int main()
{
   ...
   for(int t=0;t<T;t++)
   {
       ...
       int n,m;
       cin>>n>>m;
       int autorickshaw[n],cab[m];
       for(int i=0;i<n;i++) cin>>autorickshaw[i];
       for(int i=0;i<m;i++) cin>>cab[i];
       int autosum=0;
       for(int i=0;i<n;i++)
       {
           autosum+=min(c1*autorickshaw[i],c2);
       }
       int automin=min(autosum,c3);
       int cabsum=0;
       for(int i=0;i<m;i++)
       {
           cabsum+=min(c1*cab[i],c2);
       }
       int cabmin=min(cabsum,c3);
       int minvalue=min(automin+cabmin,c4);
       cout<<minvalue;
   }
   return 0;
}